    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var isAlreadyExist = _userManager.Users.Any(a => a.Email == model.Email);
                ...
                if (result.Succeeded)
                {
                    ...
                    var role = await _roleManager.FindByIdAsync("987b68fe-3498-43a7-b366-7a9ae59cba58"); //id of ApplicantRole
        ...
        private async Task<string> SendEmailConfirmationTokenAsync(ApplicationUser user, string subject)
        {
            ...
            // Set the expiration time to 15 minutes from the current time
            DateTime expirationTime = DateTime.UtcNow.AddMinutes(30);
            

            var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code, expirationTime }, Request.Scheme);

            string strTemplate = string.Empty;
            using (StreamReader SourceReader = System.IO.File.OpenText(_env.WebRootFileProvider.GetFileInfo("accountconfirmation.html")?.PhysicalPath))
            {
                var str = SourceReader.ReadToEnd();
                str = str.Replace("@@EMAIL", user.Email);
                str = str.Replace("@@CALLBACKURL", callbackUrl);
                strTemplate = str;
            }

            await _emailService.SendAsync(user.Email, subject, strTemplate, true);
        ...
        public ActionResult VerifyEmail(string email)
        {
            if (!string.IsNullOrEmpty(email))
            {
                ...
                // Replace characters between the first and last character with asterisks
                for (int i = 1; i < username.Length - 1; i++)
                {
                    username = username.Remove(i, 1).Insert(i, "*");
                }

                // Reconstruct the modified email address
                string modifiedEmail = $"{username}@{domain}";
                ViewData["ModifiedEmail"] = modifiedEmail;
        ...
        [AllowAnonymous]
        public async Task<ActionResult> ConfirmEmail(string userId, string code, DateTime expirationTime)
        {
            

            if (userId == null || code == null)
            {
                ViewBag.errorMessage = "Sorry, the link you provided has already expired";
                return View("Error");
            }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            var googleReCaptcha = await _googleCaptchaService.VerifyReCaptcha(model.Token);
            ...
            if (result.IsNotAllowed)
            {
                ...
                ViewBag.errorMessage = "You must have a confirmed email to log on. "
                              + "The confirmation token has been resent to your email account.";
                ModelState.AddModelError("", "You must have a confirmed email to log on. The confirmation token has been resent to your email account.");
                return View(model);
                //return RedirectToAction("Login", new { errorMessage = ViewBag.errorMessage });
            }
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var callbackUrl = Url.Action(
                    "ResetPassword",
                    "Account",
                    new { email = model.Email, code, expirationTime  },
                    protocol: Request.Scheme);

                var strTemplate = string.Empty;
                using (StreamReader SourceReader = System.IO.File.OpenText(_env.WebRootFileProvider.GetFileInfo("passwordreset.html")?.PhysicalPath))
                {
                    var str = SourceReader.ReadToEnd();
                    str = str.Replace("@@EMAIL", user.Email);
                    str = str.Replace("@@CALLBACKURL", HtmlEncoder.Default.Encode(callbackUrl));
                    strTemplate = str;
                }
        ...
        [AllowAnonymous]
        public ActionResult ResetPassword(string code = null, string email = null, DateTime? expirationTime = null)
        {
            ...
            {
                if (expirationTime is null || DateTime.UtcNow > expirationTime)
                {
                    ViewBag.errorMessage = "Sorry, the link you provided has already expired";
                    Log($"Visit ResetPassword page with error: Code is expired for password reset");
                    return View("Error");
                }
